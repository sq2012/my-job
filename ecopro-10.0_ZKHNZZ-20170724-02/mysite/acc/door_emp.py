...
def index(request):#门禁权限组设置--按人员设置
	if request.method=='GET':
		...
		colmodels= [
				...
				{'name':'door_name','index':'','width':150,'label':unicode(AccDoor._meta.get_field('door_name').verbose_name)},
				{'name':'device','index':'','width':150,'label':unicode(AccDoor._meta.get_field('device').verbose_name)}
		...
		colmodels_level= [
				...
				{'name':'Gender','sortable':False,'width':40,'search':False,'label':unicode(employee._meta.get_field('Gender').verbose_name)},
				{'name':'DeptName','sortable':False,'index':'DeptID__DeptName','width':200,'label':unicode(_('department name'))},
				{'name':'Card','sortable':False,'width':60,'label':unicode(employee._meta.get_field('Card').verbose_name)},
